...
> **Authentication**: All endpoints (except auth + health) require header: `Authorization: Bearer <token>`  
...
|---|---|---|---|
| POST | `/register` | `{ email, password, fullName }` | Đăng ký (tự tạo default categories + ví Cash). Password phải có ít nhất 8 ký tự, chữ hoa, chữ thường, số và ký tự đặc biệt. |
| POST | `/login` | `{ email, password }` | Trả về `{ accessToken, tokenType, user }`. Rate limit: 5 requests/phút. |
...
| Method | Path | Mô tả |
|---|---|---|
| GET | `/` | `totalBalance`, `monthIncome`, `monthExpense`, `pendingAmount`, `moneySources[]`, `recentTransactions[]`, `categoryBreakdown[]`. Cache 5 phút. |
...
| POST | `/` | `{ amount, period, periodStart, periodEnd, categoryId?, alertThreshold }` | Tạo. `alertThreshold` mặc định 80. |
| PUT | `/{id}` | — | Cập nhật |
| DELETE | `/{id}` | — | Soft delete |
...
| Method | Path | Mô tả |
|---|---|---|
| GET | `/overview` | Giao dịch hôm nay / tuần / tháng (count + income + expense). Cache 5 phút. |
| GET | `/recent` | 50 giao dịch gần nhất. Cache 5 phút. |
| GET | `/statistics` | Thống kê theo category, average, largest transaction. Cache 10 phút. |
| GET | `/trend` | Trend theo ngày (`?days=7`). Cache 5 phút. |

### Health — `/api/v1/health`

```
GET /api/v1/health   →  200 OK  (không cần auth)
```
...
| Biến | Mô tả | Default (local) | Required Prod |
|---|---|---|---|
| `CORS_ORIGINS` | Allowed origins (comma-separated) | `http://localhost:3000,http://localhost:5173,http://localhost:8080` | ✅ Yes |
| `DB_URL` | JDBC URL PostgreSQL | `jdbc:postgresql://localhost:5432/expense_db` | ✅ Yes |
| `DB_USERNAME` | DB user | `postgres` | ✅ Yes |
| `DB_PASSWORD` | DB password | `123456` | ✅ Yes |
| `REDIS_HOST` | Redis host | `localhost` | No |
| `REDIS_PASSWORD` | Redis password | _(trống)_ | ✅ Yes |
| `JWT_SECRET` | Secret ký JWT (Base64, >= 256 bit) | _(dev value)_ | ✅ Yes (no default) |
| `JWT_EXPIRATION_MS` | Token TTL milliseconds | `86400000` (24h) | No |
| `MAIL_USERNAME` | Gmail address | — | ✅ Yes |
| `MAIL_PASSWORD` | Gmail App Password | — | ✅ Yes |
| `MAIL_FROM` | From address trong email | — | ✅ Yes |

> **Prod**: 
> - Không bao giờ commit file `.env.prod`
> - Generate JWT_SECRET bằng: `openssl rand -base64 64`
> - `JWT_SECRET` **không có default** trong production, phải set bắt buộc
> - `CORS_ORIGINS` phải set domain thật của bạn
...
# Register với password policy mới (cần chữ hoa, chữ thường, số, ký tự đặc biệt)
curl -X POST http://your-server/api/v1/auth/register \
...
# Test rate limiting (thử 6 lần nhanh → lần thứ 6 sẽ 429 Too Many Requests)
for i in {1..6}; do
  curl -X POST http://your-server/api/v1/auth/login \
    -H "Content-Type: application/json" \
...
Tất cả controller phải dùng `/api/v1/` prefix:
```java
@RestController
@RequestMapping("/api/v1/transactions")  // ĐÚNG
@RequestMapping("/api/transactions")      // SAI
```
...
Khi tạo/sửa/xóa Transaction, cache phải bị xóa. Dùng constants từ `CacheKeyConstants`:
```java
// ĐÚNG - dùng constants
...
Register endpoint validate password mạnh:
- Tối thiểu 8 ký tự
- Ít nhất 1 chữ hoa
...
Sử dụng annotation `@ValidPassword` cho password fields.

### Rate Limiting

Authentication endpoints (`/login`, `/register`) có rate limit 5 requests/phút per IP để chống brute force. Filter tự động detect IP qua `X-Forwarded-For` header.
...
Với `@Builder`, dùng `@Builder.Default` cho fields có giá trị mặc định:
```java
@Builder.Default
private Integer alertThreshold = 80;  // ĐÚNG
```
...
**QUAN TRỌNG**: Frontend phải dùng `/api/v1/` prefix.

Update `frontend/src/services/api.js`:
```javascript
const API_BASE_URL = '/api/v1';  // ĐÚNG
const API_BASE_URL = '/api';      // SAI - deprecated
```

Axios interceptor tự gắn JWT header và redirect về `/login` khi nhận 401.
