...
public class MonthlyBalanceController {
    ...
    public ResponseEntity<ApiResponse<MonthlyBalanceResponse>> createOrUpdate(
        ...
        MonthlyBalanceResponse balance = monthlyBalanceService.createOrUpdate(userPrincipal.getId(), request);
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.success("Monthly balance saved", balance));
    }
    ...
    public ResponseEntity<ApiResponse<MonthlyBalanceResponse>> getCurrent(
            @AuthenticationPrincipal UserPrincipal userPrincipal) {
        MonthlyBalanceResponse balance = monthlyBalanceService.getCurrentBalance(userPrincipal.getId());
